...
(defmethod handle-read ((obj mud-guestbook) reader)
  ...
  (character-send-message reader (guestbook-format-entries obj))
  t)
...
(defmethod handle-write ((obj mud-guestbook) writer message)
  ...
  (character-send-message writer "You write your message in the guestbook.")
  (let ((room (object-location writer)))
    (when room
      (loop for other in (container-all-objects room) do
        (when (and (typep other 'mud-character)
                   (not (eq other writer)))
          (character-send-message other (format nil "~A writes a message in ~A."
                                             (object-name writer)
                                             (object-name obj)))))))
  t)
